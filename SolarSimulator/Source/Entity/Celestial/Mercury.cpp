...
namespace Simulator
{
	Mercury::Mercury(std::string pModelPath, std::string pShaderPath)
	{
	...
	void Mercury::Render(Camera & pCamera)
	{

		m_Model.GetModelMatrix() = glm::mat4(1.0f);
		m_Model.GetModelMatrix() = glm::scale(m_Model.GetModelMatrix(), glm::vec3(0.8f));
